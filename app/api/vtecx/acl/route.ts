...
export const PUT = async (req:NextRequest):Promise<Response> => {
  console.log(`[api acl] start. x-requested-with=${req.headers.get('x-requested-with')}`)
  ...
  try {
    if (type === 'add') {
      resJson = await vtecxnext.addacl(data)
      resStatus = resJson ? 200 : 204
    } else if (type === 'remove') {
      resJson = await vtecxnext.removeacl(data)
      resStatus = resJson ? 200 : 204
    } else {
      console.log(`[api acl] invalid type. type=${type}`)
      throw new ApiRouteTestError(400, `[api acl] invalid type. type=${type}`)
    }

  } catch (error) {
    let resErrMsg:string
    if (error instanceof VtecxNextError || error instanceof ApiRouteTestError) {
      console.log(`[api acl] Error occured. status=${error.status} ${error.message}`)
      resStatus = error.status
      resErrMsg = error.message
    } else {
      console.log(`[api acl] Error occured. (not VtecxNextError) ${error}`)
      resStatus = 503
  ...
  console.log(`[api acl] end. resJson=${resJson}`)
