...
export const PUT = async (req:NextRequest):Promise<Response> => {
  console.log(`[api setids] start. x-requested-with=${req.headers.get('x-requested-with')}`)
  ...
  try {
    ...
    console.log(`[api setids] key=${key} num=${num} ${targetservice ? 'targetservice=' + targetservice : ''}`)

    resMessage = String(await vtecxnext.setids(key, num, targetservice))
    resStatus = 200
  } catch (error) {
    if (error instanceof VtecxNextError) {
      console.log(`[api setids] Error occured. status=${error.status} ${error.message}`)
      resStatus = error.status
      resMessage = error.message
    } else {
      console.log(`[api setids] Error occured. (not VtecxNextError) ${error}`)
      resStatus = 503
